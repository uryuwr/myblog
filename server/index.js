...
// 语音转文字服务配置
const WHISPER_URL = process.env.WHISPER_URL || 'http://localhost:5000/transcribe';
const GLM_API_KEY = process.env.GLM_API_KEY || '';
const GLM_ASR_URL = 'https://open.bigmodel.cn/api/paas/v4/audio/transcriptions';

// 配置 multer 用于处理文件上传
const upload = multer({ 
  storage: multer.memoryStorage(),
  limits: { fileSize: 25 * 1024 * 1024 } // 限制 25MB
});
...
app.post('/api/speech-to-text', upload.single('file'), async (req, res) => {
  ...
  console.log(`🎤 语音识别请求 - 文件大小: ${req.file.buffer.length} bytes, 类型: ${mimeType}`);

  // 使用本地 Whisper 服务
  try {
    console.log(`🔄 尝试本地 Whisper 服务: ${WHISPER_URL}`);

    // 保存临时文件，然后发送给 Whisper
    const tempDir = tmpdir();
    const tempId = Date.now();
    const inputExt = mimeType === 'audio/webm' ? '.webm' : '.mp3';
    const inputPath = join(tempDir, `input_${tempId}${inputExt}`);
    ...
    if (whisperResponse.data.success && whisperResponse.data.text) {
      console.log(`✅ Whisper 识别成功: ${whisperResponse.data.text}`);
      return res.json({
        success: true,
      ...
      console.log(`❌ Whisper 返回无效响应`);
      return res.status(500).json({ error: 'Whisper 识别失败，请重试' });
    }
  } catch (error) {
    console.error(`❌ Whisper 服务错误: ${error.message}`);
    return res.status(503).json({ 
      error: 'Whisper 服务不可用，请确保 whisper-server 已启动',
